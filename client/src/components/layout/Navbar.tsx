...
import { LogOut, User as UserIcon, ShieldCheck, Sparkles, BookOpen, Home } from 'lucide-react';
        ...
        <div className="flex items-center space-x-4">
          {user ? (
            <div className="flex items-center space-x-3">
              {/* Interactive Clickable User Badge */}
              <button
                onClick={() => setIsProfileModalOpen(true)}
                className="flex items-center space-x-3 group text-left cursor-pointer p-1 rounded-xl hover:bg-slate-900 transition-colors"
                title="View User Profile Details"
              >
                <div className="text-right hidden sm:block">
                  <p className="text-sm font-bold text-white group-hover:text-cyan-300 transition-colors leading-tight">{user.name}</p>
