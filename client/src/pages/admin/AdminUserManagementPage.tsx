...
export const AdminUserManagementPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Admin Hero Header */}
      <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2 text-xs font-semibold text-purple-600 dark:text-purple-400 mb-1">
            <ShieldCheck className="w-4 h-4 text-purple-600 dark:text-purple-400" />
            <span>AICTE System Administrator RBAC Governance Control Panel</span>
          </div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">User & Role Permission Management</h1>
      ...
      {/* Controls Bar */}
      <div className="bg-slate-50/80 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3 w-full sm:w-auto">
          <select
            ...
            className="px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:border-brand-500 font-medium"
          >
      ...
      <div className="bg-slate-50/80 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <Users className="w-4 h-4 text-brand-600 dark:text-brand-400" /> Registered AICTE Governance Portal Users ({filteredUsers.length})
          </h3>
        </div>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60">
              {filteredUsers.map((u) => (
                <tr key={u.id} className="hover:bg-slate-100/40 dark:hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="p-3">
                    <div className="font-semibold text-slate-800 dark:text-slate-300">{u.institution}</div>
                  ...
                  <td className="p-3">
                    <select
                      ...
                      className="px-2 py-1 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded text-xs font-mono font-bold text-cyan-700 dark:text-cyan-400"
                    >
                  ...
                  <td className="p-3">
                    {u.status === 'ACTIVE' ? (
                      ...
                      <span className="inline-flex items-center gap-1 font-bold text-rose-700 dark:text-rose-400 text-[11px]">
                  ...
                  <td className="p-3 text-right">
                    <button
                      ...
                      className={`px-3 py-1 rounded text-xs font-semibold transition-colors ${
                        u.status === 'ACTIVE'
                          ? 'bg-rose-50 dark:bg-rose-500/10 hover:bg-rose-500/20 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-500/30'
                          : 'bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/30'
                      }`}
                    >
