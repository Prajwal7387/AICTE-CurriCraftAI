    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xl">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-400 bg-slate-950 px-3 py-2 rounded-xl border border-slate-800">
          <span className="font-bold text-white">{curricula.length}</span> Total Curricula tracked in governance pipeline
        </div>
      </div>
            ...
            <div className="space-y-3">
              {pendingCurricula.length > 0 ? (
                pendingCurricula.map((curr) => (
                  <div key={curr._id} className="p-4 bg-slate-950 border border-purple-500/20 hover:border-purple-500/40 rounded-xl space-y-3 shadow-md transition-all">
                    <div>
                      ...
                      <h4 className="text-sm font-semibold text-white leading-snug mt-1">{curr.title}</h4>
                      <p className="text-[11px] text-slate-400 mt-0.5">{curr.branch} • {curr.totalCredits} Credits</p>
                      {curr.reviewer && (
                    ...
                    <div className="flex items-center space-x-2 pt-2 border-t border-slate-800">
                      <button
                        ...
                        className="flex-1 py-1.5 bg-emerald-600/20 hover:bg-emerald-600 text-emerald-400 hover:text-white text-xs font-bold rounded-lg border border-emerald-500/30 flex items-center justify-center gap-1 transition-all"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 py-1.5 bg-amber-600/20 hover:bg-amber-600 text-amber-400 hover:text-white text-xs font-bold rounded-lg border border-amber-500/30 flex items-center justify-center gap-1 transition-all"
                      >
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <h3 className="text-xs font-bold text-amber-400 uppercase tracking-wider flex items-center gap-1.5">
                <AlertTriangle className="w-4 h-4 text-amber-400" /> Denied / Revision Requested
              </h3>
            ...
            <div className="space-y-3">
              {deniedCurricula.length > 0 ? (
                deniedCurricula.map((curr) => (
                  <div key={curr._id} className="p-4 bg-slate-950 border border-amber-500/30 rounded-xl space-y-3 shadow-md transition-all">
                    <div>
                      <div className="flex items-center justify-between">
                        <span className="font-mono text-[10px] text-amber-400 font-bold">{curr.code}</span>
                        <span className="px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300 text-[9px] font-bold border border-amber-500/30 flex items-center gap-1">
                          <XCircle className="w-3 h-3 text-amber-400" /> Revision Requested
                        </span>
                      </div>
                      <h4 className="text-sm font-semibold text-white leading-snug mt-1">{curr.title}</h4>
                      <p className="text-[11px] text-slate-400 mt-0.5">{curr.branch} • {curr.totalCredits} Credits</p>
                    ...
                    {/* Feedback box */}
                    <div className="p-2.5 bg-amber-950/30 border border-amber-500/20 rounded-lg text-xs space-y-1">
                      <span className="text-[10px] font-bold uppercase text-amber-400 flex items-center gap-1">
                        <MessageSquare className="w-3 h-3 text-amber-400" /> Bureau Feedback Notes:
                      </span>
                    ...
                    <button
                      ...
                      className="w-full py-1.5 bg-slate-800 hover:bg-amber-600 text-slate-300 hover:text-white text-xs font-bold rounded-lg border border-slate-700 hover:border-amber-500 transition-all flex items-center justify-center gap-1.5"
                    >
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <h3 className="text-xs font-bold text-emerald-400 uppercase tracking-wider flex items-center gap-1.5">
                <CheckCircle className="w-4 h-4 text-emerald-400" /> Approved Curricula
              </h3>
            ...
            <div className="space-y-3">
              {approvedCurricula.length > 0 ? (
                approvedCurricula.map((curr) => (
                  <div key={curr._id} className="p-4 bg-slate-950 border border-emerald-500/30 rounded-xl space-y-3 shadow-md transition-all">
                    <div>
                      <span className="font-mono text-[10px] text-emerald-400 font-bold">{curr.code}</span>
                      <h4 className="text-sm font-semibold text-white leading-snug mt-1">{curr.title}</h4>
                      <p className="text-[11px] text-slate-400 mt-0.5">Approved by AICTE Peer Review Committee</p>
                    </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <h3 className="text-xs font-bold text-cyan-400 uppercase tracking-wider flex items-center gap-1.5">
                <Globe className="w-4 h-4 text-cyan-400" /> Published Live
              </h3>
            ...
            <div className="space-y-3">
              {publishedCurricula.length > 0 ? (
                publishedCurricula.map((curr) => (
                  <div key={curr._id} className="p-4 bg-slate-950 border border-slate-800 hover:border-cyan-500/30 rounded-xl space-y-2 shadow-md transition-all">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-[10px] text-cyan-400 font-bold">{curr.code}</span>
                      <span className="text-[10px] text-emerald-400 font-semibold uppercase flex items-center gap-1 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">
                        <Sparkles className="w-3 h-3 text-emerald-400" /> Live
                      </span>
                    </div>
                    <h4 className="text-sm font-semibold text-white leading-snug">{curr.title}</h4>
                    <p className="text-[11px] text-slate-400">Publicly accessible for university adoption</p>
                  </div>
                ))
              ) : (
                <div className="p-6 text-center text-slate-500 text-xs border border-dashed border-slate-800 rounded-xl">
      ...
      {requestEditsCurr && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-slate-900 border border-amber-500/40 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in duration-150">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div className="flex items-center gap-2 text-amber-400 font-bold">
                <XCircle className="w-5 h-5 text-amber-400" />
                <h3 className="text-lg text-white">Request Edits / Issue Revision Note</h3>
              </div>
              <button
            ...
            <form onSubmit={handleConfirmRequestEdits} className="space-y-4">
              <div>
                <p className="text-xs text-slate-300 font-semibold mb-1">Target Curriculum:</p>
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800">
                  <p className="font-mono text-xs text-amber-400 font-bold">{requestEditsCurr.code}</p>
                  <p className="text-xs text-white font-bold">{requestEditsCurr.title}</p>
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">Quick Select Common Feedback Notes:</label>
                <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                  {presetFeedbackOptions.map((opt, idx) => (
                    <button
                      ...
                      className="w-full text-left p-2 rounded-lg bg-slate-950 hover:bg-amber-950/40 border border-slate-800 hover:border-amber-500/40 text-[11px] text-slate-300 hover:text-amber-200 transition-all"
                    >
                      + {opt}
                    </button>
                  ))}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">Detailed Rejection / Revision Comments *</label>
                <textarea
                  rows={4}
                  required
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-amber-500 leading-relaxed"
                />
              </div>
