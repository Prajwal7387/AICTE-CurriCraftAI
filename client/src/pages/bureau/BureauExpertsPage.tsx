    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400">Manage expert credentials and assign curriculum peer evaluation duties</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:border-violet-500 w-56"
            />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {filteredExperts.map((exp) => (
          <div key={exp.id} className="glass-card-glow p-5 rounded-2xl space-y-4 hover:border-violet-500/40 transition-all">
            ...
            <div>
              <h4 className="text-base font-bold text-slate-900 dark:text-white">{exp.name}</h4>
              <p className="text-xs text-slate-600 dark:text-slate-400">{exp.title} • {exp.institution}</p>
      ...
      {filteredExperts.length === 0 && (
        <div className="p-12 text-center bg-slate-50/60 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-2">
          <UserCheck className="w-10 h-10 text-slate-600 mx-auto" />
          <p className="text-sm font-semibold text-slate-800 dark:text-slate-300">No subject experts match your filter criteria.</p>
      ...
      {isEmpanelModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md">
          <div className="bg-slate-50 dark:bg-slate-900 border border-violet-500/30 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in duration-150">
            <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-2 text-violet-400 font-bold">
                <Plus className="w-5 h-5 text-violet-400" />
                <h3 className="text-lg text-slate-900 dark:text-white">Empanel New Subject Expert</h3>
              </div>
              <button
            ...
            <form onSubmit={handleEmpanelSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:border-violet-500"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-800 dark:text-slate-300 mb-1">Academic Designation</label>
                  <select
                    value={newExpert.title}
                    onChange={(e) => setNewExpert({ ...newExpert, title: e.target.value })}
                    className="w-full px-3 py-2.5 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:border-violet-500"
                  >
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-800 dark:text-slate-300 mb-1">Domain Department</label>
                  <select
              ...
              <div>
                <label className="block text-xs font-bold text-slate-800 dark:text-slate-300 mb-1">Institution / University *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-slate-800 dark:text-slate-300 mb-1">Official Email Address *</label>
                <input
                  type="email"
                  required
      ...
      {selectedExpertForAssign && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md">
          <div className="bg-slate-50 dark:bg-slate-900 border border-indigo-500/30 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in duration-150">
            <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
              <div>
                <div className="flex items-center gap-2 text-indigo-400 font-bold">
                  <BookOpen className="w-5 h-5 text-indigo-400" />
                  <h3 className="text-lg text-slate-900 dark:text-white">Assign Peer Review Task</h3>
                </div>
                <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">
                  Assigning to <span className="text-slate-900 dark:text-white font-semibold">{selectedExpertForAssign.name}</span> ({selectedExpertForAssign.institution})
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleAssignSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-800 dark:text-slate-300 mb-1">Select Model Curriculum for Review *</label>
                <select
                  value={selectedCurriculumId}
                  onChange={(e) => setSelectedCurriculumId(e.target.value)}
                  className="w-full px-3.5 py-2.5 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500"
                >
                  {curricula.map((curr) => (
              ...
              <div>
                <label className="block text-xs font-bold text-slate-800 dark:text-slate-300 mb-1">Evaluation Target Deadline</label>
                <select
              ...
              <div>
                <label className="block text-xs font-bold text-slate-800 dark:text-slate-300 mb-1">Bureau Review Notes / Specific Instructions</label>
                <textarea
                  rows={3}
                  ...
                  className="w-full px-3.5 py-2.5 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                />
              </div>
