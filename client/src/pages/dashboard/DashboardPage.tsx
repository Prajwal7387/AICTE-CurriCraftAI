...
export const DashboardPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Welcome Banner */}
      <div className="bg-gradient-to-r from-brand-900/60 via-slate-900 to-slate-900 border border-brand-500/20 rounded-2xl p-6 shadow-xl relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-4 flex flex-col justify-between">
          ...
          <p className="text-2xl font-bold text-white mt-2">{curricula.length || stats.totalCurricula}</p>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-4 flex flex-col justify-between">
          ...
          <p className="text-2xl font-bold text-white mt-2 flex items-baseline gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-900/80 border border-slate-800 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-400">Affiliated engineering colleges adopting model curricula</p>
            </div>
          </div>
          <div className="h-56">
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 flex flex-col justify-between">
          <div>
            <h3 className="text-sm font-semibold text-white">Department Share</h3>
            <p className="text-xs text-slate-400">Distribution across major branches</p>
          </div>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div>
            <h3 className="text-base font-bold text-white">AICTE Model Curriculum Registry</h3>
            <p className="text-xs text-slate-400">Manage and edit active model curriculum documents</p>
          </div>
          ...
          <div className="relative w-full md:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 bg-slate-950 border border-slate-800 rounded-lg text-xs text-white focus:outline-none focus:border-brand-500"
            />
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {filteredCurricula.length > 0 ? (
                filteredCurricula.map((curr) => (
                  <tr key={curr._id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="p-3 font-medium text-white">
                      <div className="font-semibold text-sm">{curr.title}</div>
                      <span className="text-[11px] font-mono text-brand-400">{curr.code}</span>
                    </td>
                    ...
                    <td className="p-3 text-right space-x-2">
                      <button
                        ...
                        className="px-3 py-1 bg-brand-600/20 hover:bg-brand-600 text-brand-300 hover:text-white rounded text-[11px] font-medium transition-colors border border-brand-500/30 inline-flex items-center gap-1"
                      >
                        <span>Workspace</span>
