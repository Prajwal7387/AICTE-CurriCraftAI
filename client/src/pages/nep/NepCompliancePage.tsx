      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-slate-50/80 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-xl p-5 text-center flex flex-col justify-center">
            ...
            <p className="text-4xl font-extrabold text-emerald-500 dark:text-emerald-400 mt-1">{report.score}%</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                >
                  ...
                  <div>
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 font-semibold">{chk.category}</span>
                    </div>
                    <p className="text-xs text-slate-800 dark:text-slate-300 font-medium mt-1 leading-relaxed">{chk.message}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="lg:col-span-5 bg-slate-50/80 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-4">
            <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <Lightbulb className="w-4 h-4 text-amber-500 dark:text-amber-400" /> Actionable Recommendations
            </h3>
            <div className="space-y-2 text-xs">
              {report.recommendations.map((rec, idx) => (
                <div key={idx} className="p-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl text-amber-950 dark:text-amber-300 font-medium leading-relaxed shadow-sm">
                  • {rec}
                </div>
              ))}
            </div>
